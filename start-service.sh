chmod 0600 "$(pwd)"/ssh_tunnel/tunnel_rsa

docker build -t go_mux_postgresql_gorm_ssh ssh_tunnel

docker start go_mux_postgresql_gorm_ssh 2>/dev/null || docker run -d \
  --name=go_mux_postgresql_gorm_ssh \
  -e TZ=Etc/UTC \
  -e "PUBLIC_KEY=ssh-rsa " \
  -p 127.0.0.1:58222:2222 \
  -e USER_NAME=tunnel \
  --restart unless-stopped \
  go_mux_postgresql_gorm_ssh

sleep 5

ssh-keygen -R '[localhost]:58222'
pkill -f "ssh -i $(pwd)/ssh_tunnel/tunnel_rsa tunnel@localhost -p 58222"
ssh -i "$(pwd)"/ssh_tunnel/tunnel_rsa tunnel@localhost -p 58222 -4 -o StrictHostKeyChecking=no -R 5432:127.0.0.1:5432 -R 4318:127.0.0.1:4318 -L 3000:127.0.0.1:3000 -fN

docker build -t go_mux_postgresql_gorm .

docker start go_mux_postgresql_gorm 2>/dev/null || docker run \
  --name=go_mux_postgresql_gorm \
  --network 'container:go_mux_postgresql_gorm_ssh' \
  go_mux_postgresql_gorm
